...
const JobAvailableAssessment = () => {
  ...
  // const fetchEmployee = () => {
  //   fetch(
  //     `https://localhost:7281/HistoryTable/HistoryDetails?roleName=Jobseeker`,
  //     {
  //       method: 'GET',
  //       headers: {
  //         Authorization: 'Bearer ' + sessionStorage.getItem('token'),
  ...
  const copyToClipboard = () => {
    copy(copyText)
    toast.success('You Copied ', copyText)
  }
  ...
  const fetchAssessment = async () => {
    try {
      const response = await fetch(
        'https://localhost:7281/api/AssessmentData/AllAssessments?rolename=Jobseeker',
        {
          headers: {
            Authorization: 'Bearer ' + sessionStorage.getItem('token'),
          },
      ...
      toast.error('Error fetching assessment:', error)
    }
  }
  ...
  const handlePostRequest = async () => {
    try {
      ...
      const response = await fetch(
        'https://localhost:7281/api/AssessmentData/GetSelectedItems?rolename=Jobseeker',
        requestOptions
      )
      if (response.ok) {
        const data = await response.json()
  ...
  const handleAddChip = () => {
    if (inputValue.trim() !== '') {
      ...
      setInputValue('')
      const selectedEmail = Array.isArray(inputValue)
        ? inputValue
        : [inputValue]
      const selectedIds = selectedEmail.map((name) => {
        const jobseeker = getAllJS.find((emp) => emp.userEmail === name)
        if (jobseeker) {
  ...
  return (
    <>
      <div className="Available-Assessment">
        <div className="select-tags">
          ...
          <div className="select1">
            <FormControl sx={{ m: 1, width: 300, marginLeft: '40px' }}>
      ...
      <h4
        className="KSA-ListOfAssessments"
        style={{
          fontFamily: 'manrope',
      ...
      <div className="List-Of-Assessments">
        ...
        <div className="ending">
          ...
          <div className="flex-prop1">
            <div
              className="margin"
              style={{ paddingRight: '50px', marginTop: '25px' }}
            >
              Go to page ----
            </div>
            <div
...
export default JobAvailableAssessment
